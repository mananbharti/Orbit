...
async function main(): Promise<void> {
  ...
  if (!['connect', 'clipboard', 'files'].includes(action ?? '')) throw new Error('Use pair <pairing.json>, connect, clipboard, or files');
  ...
  const clipboard = action === 'clipboard' || (action === 'files' && process.argv.includes('--clipboard')) ? new ClipboardClient(client, createClipboard()) : undefined;
  ...
  try { await client.start(); } catch (error) { await files?.stop(); await clipboard?.stop(); throw error; }
